...
// ==========================================
// UNIFIED CERTIFICATE COMPONENT (SCREEN VIEW)
// ==========================================
function DigitalCertificate({ data, onBack, isClientView, t, isRtl, hideIsrael, isPublicVerification = false, setIsPrinting }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 pb-24 animate-in zoom-in-95 no-print print:hidden">
      ...
      {/* Container specifically sized to display neatly on screen */}
      <div className="w-full overflow-x-auto flex justify-center pb-4 no-print scrollbar-hide">
        <div className="bg-white border-[10px] border-[#0a0a0a] p-2 shadow-2xl relative w-[210mm] max-w-full">
          <div className="border-[3px] border-[#d4af37] p-8 md:p-14 relative flex flex-col items-center text-center overflow-hidden bg-white">
            ...
            <div className="absolute top-6 right-6 text-right">
               <p className="text-[10px] font-black tracking-widest text-slate-400 uppercase leading-none m-0 mb-1">Certificate No.</p>
               <p className="text-base font-bold text-slate-800 font-mono tracking-wider leading-none m-0">{data.id}</p>
            </div>
            ...
            <div className="w-full max-w-xl mb-8 relative z-10">
              <div className="grid grid-cols-2 gap-y-4 text-left border-b border-slate-200 pb-4 mb-4" dir="ltr">
                ...
                <div className="font-bold text-slate-900 text-lg">{data.brand}</div>
                <div className="text-slate-500 text-sm uppercase tracking-widest">Model</div>
                <div className="font-bold text-slate-900 text-lg">{data.model}</div>
                {data.serialNumber && (
                  <>
                    <div className="text-slate-500 text-sm uppercase tracking-widest">Serial Number</div>
                    <div className="font-bold text-slate-900 text-lg">{data.serialNumber}</div>
                  </>
                )}
                <div className="text-slate-500 text-sm uppercase tracking-widest">Date Inspected</div>
                <div className="font-bold text-slate-900 text-lg">{new Date(data.createdAt).toLocaleDateString('en-GB')}</div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3 w-full">
                {imagesToDisplay.map(([part, url]) => (
                   <div key={part} className="flex flex-col items-center text-center">
                      <img src={url} alt={part} className="w-full h-24 object-cover border border-slate-200 rounded-md shadow-sm" style={{ objectFit: 'cover' }} />
                      <span className="mt-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">{part}</span>
                   </div>
                ))}
                {imagesToDisplay.length === 0 && (
                   <div className="col-span-4 flex flex-col items-center text-center">
                     <img src={data.image} className="w-64 h-32 object-cover border border-slate-200 rounded-md shadow-sm" style={{ objectFit: 'cover' }} />
                     <span className="mt-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">MAIN</span>
                   </div>
                )}
              </div>
            </div>
...
function MainApp() {
  ...
  // כאשר אנו מפעילים את המדפסת (דרך setIsPrinting),
  // המערכת מציגה רק את תצוגת ההדפסה הנסתרת והנקייה ומעלימה הכל.
  if (isPrinting) {
    return (
